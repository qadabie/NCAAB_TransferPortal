...
# make copy of the ncaa_players table & sort by player_id and year
players = ncaa_players.copy()
players.sort_values(by=['player_id', 'year'], inplace=True)

# Add previous year and previous team columns
players['prev_year'] = players.groupby('player_id')['year'].shift(1)
players['prev_team'] = players.groupby('player_id')['team'].shift(1)

# Create is_transfer column; returns True if player played on a different team the previous year
players['is_transfer'] = np.where((players['prev_year'] == players['year'] - 1) & (players['prev_team'] != players['team']), True, False)

# Group by 'year' and 'team' and extract value_counts() from the is_transfer column
played = players.groupby(['year', 'team'])['is_transfer'].value_counts().reset_index()

# create new Dataframe to include players if they were labeled as a transfer
ncaa_transfers = played.loc[played['is_transfer'] == True]

# Create pivot table to break out rows containing # of transfers by year to columns
# Fill NaN with 0
ncaa_transfers = ncaa_transfers.pivot(index='team', columns='year', values='count').fillna(0).reset_index()
